...
void sevenSeg_set_number(uint8_t num){
    ...
    if(num > 9)
    {
    	HAL_GPIO_WritePin(DIGIT_1_GPIO_Port, DIGIT_2_Pin, GPIO_PIN_SET);
    }
    else{
    	HAL_GPIO_WritePin(DIGIT_1_GPIO_Port, DIGIT_1_Pin, GPIO_PIN_SET);
    }

    HAL_GPIO_WritePin(GPIOA, UINT8_MAX, GPIO_PIN_SET); // Clears digit
	HAL_GPIO_WritePin(GPIOA, digit_map[num%10], GPIO_PIN_RESET);
